...
% Load Faces 
blackDir='blackFaces'; %changed the name of the folder?
whiteDir='whiteFaces';
homeDir=pwd;
...
%             blackFaceCtr=blackFaceCtr+1;
            ...
            stimStr{ind} = sprintf('Screen(''PutImage'', w, black(%d).image);', stimSel);
            corrans{ind}='Black';
        case 2
            stimSel=round(rand*(numWhiteFaces-1))+1;
%             whiteFaceCtr=whiteFaceCtr+1;
            stimName{ind}=white(stimSel).name;
            stimStr{ind} = sprintf('Screen(''PutImage'', w, white(%d).image);', stimSel);
            corrans{ind}='White';
        case 3
